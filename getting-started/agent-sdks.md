...
Same shape as Copilot: the Agent SDK spawns the Claude Code CLI as a
child process, the CLI is roughly **320 MB per platform**, and the
extension does not bundle it. The SDK on its own only looks for the CLI
beside itself — a place that is empty in an installed extension — so the
provider finds it for the SDK, in this order:

1. `CLAUDE_CLI_PATH`, if set — the task's `env:`, your shell, or the
   machine.
2. The copy beside the SDK — only a source checkout has one.
3. `claude` on `PATH`, walked to the real executable — what the npm
   global install or the native installer leaves behind.

If none of those hit, the session refuses to start and says so:
`Claude Code CLI not found ... npm install -g @anthropic-ai/claude-code`.
Before this, the same machine got the SDK's own message —
`Native CLI binary for win32-x64 not found. Reinstall
@anthropic-ai/claude-agent-sdk without --omit=optional` — which asks
you to reinstall a package you never installed.
...
tasks:
  draft:
    ...
    provider: claude
    env:
      CLAUDE_CLI_PATH: D:\tools\claude\claude.exe
    args:
      prompt: Summarise the commits since the last tag.
```

Point it at the **executable**, not at `claude.cmd` or a `.js` file.
The SDK spawns the path directly with no shell: Node refuses a `.cmd`
that way, and a `.js` would be run under `Code.exe`.

The CLI the provider finds does not have to be the same version as the
SDK inside the extension. A newer CLI with an older SDK is the normal
case for anyone who keeps Claude Code updated.
...
No `ELECTRON_RUN_AS_NODE` workaround — that one is Copilot-specific, a
consequence of its entry point being a `.js` file. The Claude CLI is a
native executable.
...
| `plan:` | a checklist file the session may tick through the `task_done` tool — one box per call, nothing else in the file touched. Set by the spec panel on implementation chats |
...
1. **A fixed baseline** — the process plumbing no child can start
   ...
   `LC_ALL`, `SHELL`, `COPILOT_CLI_PATH` and `CLAUDE_CLI_PATH`. The home directory is
...
provider does the same lookup for its CLI but hands the result to the
SDK directly rather than through the environment, so `CLAUDE_CLI_PATH`
is only ever what you set.
