...
own SDK. Seven more are built in over the
[Agent Client Protocol](https://agentclientprotocol.com) instead:
`kiro-cli`, `copilot-cli`, `claude-cli`, `gemini-cli`, `codex-cli`,
`cursor-cli` and `opencode-cli`. Each is a `provider:` an
`ai@1` task can name with nothing in `settings.json`; what you do need
is the vendor's CLI installed. `ai.acpAgents` in `settings.json` is the
door for anything else that speaks ACP, and for changing how a built-in
one is started.
...
**For Kiro.** No SDK, so `kiro-cli` is the only way in. It is built in;
install the CLI and name it.

**For Claude's or Copilot's own CLI over ACP** — `claude-cli` and
`copilot-cli`, also built in — for the one time it's useful: comparing
behaviour across agents, or standardising a team's whole setup on one
protocol rather than three vendor SDKs.

**For Gemini CLI, Codex, Cursor or OpenCode** — `gemini-cli`,
`codex-cli`, `cursor-cli`, `opencode-cli`, built in. See
[the other built-in ids](#the-other-built-in-ids) for what each needs
installed.

**For an agent this build ships no id for**, or to change the command
behind a built-in id. Both go under `ai.acpAgents`.
...
This is exactly the built-in `kiro-cli` entry, spelled out — you only
write it to change something, such as a full path to the binary.
`provider: kiro-cli` on an `ai@1` task runs on it — same session shape
...
reached. An entry under a built-in `-cli` id replaces the built-in
one wholesale.
...
`provider: claude-cli`, built in. An alternative to the built-in
`claude` provider, using the official
...
Nothing to declare. The built-in entry is `{ "command": "claude-agent-acp" }`;
write that under `ai.acpAgents.claude-cli` only to change it.
...
first. That is why the shipped id is `claude-cli`.
...
`provider: copilot-cli`, built in. An alternative to the built-in
`ghcp` provider.
...
Nothing to declare. The built-in entry is
`{ "command": "copilot", "args": ["--acp", "--stdio"] }`; write that
under `ai.acpAgents.copilot-cli` only to change it.

**Same rule as Claude above: the id cannot be `ghcp`** — that name is
the built-in provider's, and it is checked first every time. That is
why the shipped id is `copilot-cli`.
...
## The other built-in ids

Four more ship ready to name. Each needs its vendor's CLI installed and
signed in first; none is a dependency of this extension. Commands come
from the [ACP registry](https://github.com/agentclientprotocol/registry)
as of 2026-09.

| `provider:` | runs | install |
|---|---|---|
| `gemini-cli` | `gemini --acp` | `npm install -g @google/gemini-cli` |
| `codex-cli` | `codex-acp` | `npm install -g @agentclientprotocol/codex-acp` — the ACP adapter for OpenAI Codex, same idea as `claude-agent-acp` |
| `cursor-cli` | `cursor-agent acp` | Cursor's agent CLI, from cursor.com |
| `opencode-cli` | `opencode acp` | OpenCode, from opencode.ai |

None of these four has been run end to end by this project the way
Kiro, Claude and Copilot have. Read the `ACP agent '<id>' session`
block the engine logs on `initialize` (see [Others](#others) below)
before trusting one with anything unattended.
...
writing includes Cline, Devin, Goose, Qwen Code, Kimi and others beyond
the seven built in.
