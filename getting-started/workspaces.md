...
workspace- or profile-level task that has not said where it runs. The
answer is used for that one run and is not remembered.
...
| Written | Means |
|---|---|
| `cwd: none` | This acts on no folder. The run starts in the file's own project folder. |
| `cwd: ./api` | Run here. Relative to the folder the run starts in. |
| `cwd: ${{ home }}/scratch` | Same, with templating. |
...
See [Workflows that need no folder](https://local-workflows.github.io/getting-started/workflows.md#workflows-that-need-no-folder)
for a complete file.
...
One JSON record per line, appended as the run goes and read back whole
when you reopen it. A workflow or task run is one execution, so it gets
one file, named so the folder sorts into the order things happened. A
spec is not an execution — it is drafted and revised for as long as it
lives — so it is one growing file per spec, and running a phase again
appends rather than erasing what the last attempt argued.

Files rather than rows because this is the one thing nobody queries: you
can delete one run's output by hand, and a file damaged by a crash costs
that one run rather than the workspace's whole history.
