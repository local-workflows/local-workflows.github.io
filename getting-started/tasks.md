...
| `cwd` | `string` | the file's project folder | Where this file's tasks start, before any task's own `cwd:`. Relative to the folder the run starts in unless absolute. `none` means it acts on no folder — see [Workflows that need no folder](/getting-started/workflows/#workflows-that-need-no-folder). |
...
| `foreach` | expression \| list | | Run once per item, one after another. See [One task, many items](#one-task-many-items). |
| `foreachAs` | `string` | `item` | The name the item is read by: `${{ item }}`, `${{ itemIndex }}`. |
| `failFast` | `boolean` | `true` | Stop the loop at the first failing item. `false` tries every item and fails the task if any did. |
| `cwd` | `string` | the file's | Relative to the level above it unless absolute. `none` means this task acts on no folder — see [Workflows that need no folder](/getting-started/workflows/#workflows-that-need-no-folder). |
...
`foreach:` runs a task once per item of a list. The items run one
after another, never side by side — they share one working tree.
...
- **The list** is one expression that resolves to a list — a
  `type: list` param, a path into an artifact — or a literal YAML list.
  Anything else fails the task, naming what it resolved to. An empty
  list is not a failure: the task logs *Nothing to iterate* and succeeds.
- **The item** is `${{ item }}`, a path into it
  `${{ item.releaseId }}`, its position
  `${{ itemIndex }}`. `foreachAs: release` renames
  both: `${{ release }}`, `${{ releaseIndex }}`.
- **Each item** gets its own attempt, its own `retries:`, its own lines
  in the log (`Item 2/9: …`), and under `trigger: manual` its own gate
  card showing that item's resolved args. Nine releases, nine
  decisions.
- **The artifact collects**: `artifact: CREATED` stores
  `{ items: [...] }` in item order, read as
  `${{ run.context.CREATED.items }}` or
  `${{ run.context.CREATED.items[0].releaseId }}`.
  It is written after every item, which is what a restart reads: a run
  parked at item 4's gate resumes at item 4, with 1 to 3 taken from the
  store instead of run again. So a task that gates per item must
  declare `artifact:`; the parser refuses one that does not.
- **Failure**: `failFast` (default) stops the loop at the first failing
  item. `failFast: false` tries every item and fails the task if any
  did. Either way `continueOnError:` then decides whether the run goes
  on. A rejected gate skips the rest of the loop and the rest of the
  run, as any rejected gate does. Stop stops before the next item.
- `if:` is evaluated once, before the loop, and cannot read the item.
- A stage does not take `foreach:`. A stage is a barrier; the loop
  belongs on the task.
...
1. Directory anchors and run stamps — `workspaceFolder`, `workspaceConfig`, `cwd`, `home`, `dateToday`, `runId`, …
2. `vars:` — may read anchors, stamps and each other
3. `dotenv:` files, then `env:` — may read anchors, stamps and `vars`
...
Six more bare names, stamped onto the run **the moment it starts** and
frozen: every task, the `label:` and a gate preview all read the same
instant, and a run resumed tomorrow keeps saying the day it started.
They show under **Stamps** on the run panel's Data tab.

| Stamp | Value | Example |
|---|---|---|
| `${{ dateToday }}` | the run's local date | `2026-09-05` |
| `${{ dateNow }}` | local date and time, ISO | `2026-09-05T15:42:10` |
| `${{ dateUtcNow }}` | UTC date and time, ISO | `2026-09-05T10:12:10Z` |
| `${{ timestamp }}` | local time, compact | `20260905-154210` |
| `${{ runId }}` | the run's own id | — |
| `${{ platform }}` | the OS family | `windows` \| `macos` \| `linux` |

`dateNow` and `dateUtcNow` carry colons, which Windows forbids in file
names — `path: report-${{ dateNow }}.md` fails there. Name files with
`timestamp` or `dateToday` instead.
...
| A `${{ }}` path that misses inside a value | fails, naming the deepest node that resolved and what it has |
| `shell: cmd` off Windows | validation error before running |
| `foreach:` resolving to something that is not a list | fails the task, naming what it resolved to |
| `foreach:` resolving to an empty list | not a failure — logs `Nothing to iterate`, succeeds |
| `foreach:` with `trigger: manual` and no `artifact:` | parse error — the artifact is where a restart reads which items are done |
| `foreachAs:` naming something an expression already resolves | parse error |
| `foreachAs:` or `failFast:` without `foreach:` | parse error |
| A failing item | stops the loop (`failFast`, the default); `failFast: false` tries every item and fails the task if any did |
| A rejected item gate | the rest of the loop and the rest of the run are skipped |
