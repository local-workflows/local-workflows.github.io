...
last — or marks its plan as **actionable**, and the gate is implied
after it:
...
An actionable stage's document is worked from its own page: every
checkbox row gets a live box and a ▶, and the implementation starts
from there — see [Implement](https://local-workflows.github.io/getting-started/sdd/index.md#5-implement).
The output gate, `implement`, is added after it by the parser, the way
`intake` is added in front, and is never shown as a phase: its chats
are listed under the actionable stage in the sessions pane, and its
notes are on that stage's own Logs tab. One actionable stage per pipeline, always the last drafting
stage; on any other position, or on a gate, the style fails to load.

The older spelling still works: list `implement` yourself, last, with
no `actionable` anywhere, and it is drawn as a phase of its own with a
Tasks tab. Listing it *and* marking a stage actionable is refused —
they are one thing said twice.
...
reach*. Listed by hand, it must be the last entry — anywhere else, the
style fails to load, because the stages after a terminal one could never
be reached.
...
| `presets` | list | Optional. Starting sentences for every drafting phase's chat box, ahead of each stage's own `presets:` below — same shape, same rules. Write a house sentence once here instead of on every stage. Declared, the two built-in sentences are dropped for every phase. |
...
Those ten, and no others. There is no `frame:` key any more — the wrapper
around a phase's instructions is not made of nameable paragraphs. It is
one of the four whole-message files in the style's `prompts/` folder
(`first.md`, `rewrite.md`, `rewrite-note.md`, `implement.md`), and rewording it means
...
document. There is no `revise:` key either: a revise re-runs the phase's
own prompt over the document it already wrote, handing it that document as
`current` and whatever the person typed in the chat box as a note.
...
| `stages` | `string[]` | Ordered stage ids — this is the pipeline. A type that skips design just omits it here. `intake` is added automatically and never listed. `implement` is added automatically after a stage marked `actionable`; a style with no such stage may list it by hand, last. Neither, and the pipeline ends at your last drafting phase. |
...
| `actionable` | `boolean` | Optional. This stage's document is the plan: its checkbox rows get a live box and a ▶, and the implementation is started from its page. The output gate is implied after it and never shown. Only on the last drafting stage of a pipeline, one per pipeline; never on a gate. |
| `presets` | list | Optional. Starting sentences for this phase's chat box — the split button beside Send. Each entry is `label:` (the menu item) and `text:` (what fills the box). Picking one only fills the box; you still read it, edit it and press Send, and it goes as the note of a revise pass. Added after the style's own top-level `presets:`. Declared here or there, the two built-in sentences (*Prompt changed*, *Prev doc(s) changed*) are dropped for this phase; declared nowhere, they stay. The text may read the spec variables. Not on `intake` or `implement`. |
...
four whole files, in the style's own `prompts/` folder, one for each
kind of run:

| File | Runs when |
|---|---|
| `prompts/first.md` | no document exists yet — a first draft |
| `prompts/rewrite.md` | the document exists and the chat box was left empty |
| `prompts/rewrite-note.md` | the document exists and something was typed into the chat box |
| `prompts/implement.md` | the `implement` stage — tasks handed to an agent, no document written |
...
how a revise is worded and leaves the other three as the built-in has
...
variables](#spec-variables) — plus a few more, answered only here:

| Variable | Is |
|---|---|
| `${{ phase.target }}` | The document this phase writes, in backticks. Not in `prompts/implement.md`, which writes none. |
| `${{ phase.note }}` | What a person typed into the chat box. Only in `prompts/rewrite-note.md`. |
| `${{ phase.tasks }}` | The tasks handed to this run, one row each — `- 1.1 Title (wave 0)`; the wave is left off when the plan declared no dependency graph. Only in `prompts/implement.md`. |
...
1. Run **Local Workflows: Eject Style** and pick the built-in `custom`
   ...
   (`first.md`, `rewrite.md`, `rewrite-note.md`, `implement.md` — whichever
   the style ships) and `grammar.md`, into the scope you choose — your **user
   ...
   `.local-workflows/styles/kiro/` (or `spec-kit/`) yourself. Those two
   pages are hand-written worked examples, not generated — the page is
   the only copy, so what you paste is what runs.
