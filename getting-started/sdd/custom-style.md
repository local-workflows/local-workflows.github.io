...
and notes appear on the actionable stage's own Logs and AI Session
tabs. One actionable stage per pipeline, always the last drafting
...
| `presets` | list | Optional. Starting sentences for every drafting phase's revise box, ahead of each stage's own `presets:` below — same shape, same rules. Write a house sentence once here instead of on every stage. Declared, the two built-in sentences are dropped for every phase. |
