...
| one command in backticks | run in the editor's integrated terminal, exactly as written, standing in the workspace folder that holds the spec | a `>_` button |
...
**never handed to an agent** — not by its own ▶, not by Start. The
engine also never ticks it: whichever way it was done, the box is the
person's to click.

**Where a command runs.** The terminal opens in the workspace folder
that holds the spec — the folder the plan's paths are relative to, so
`git -C orders-api …` finds `orders-api` — never in the window's root,
which in a `.code-workspace` is somewhere else. It is named
`Local Workflows · <folder>` and reused for the next command from the
same folder; a spec in another folder gets a terminal of its own, so a
reused terminal is always standing where the command expects.

**What a workflow is given.** A workflow started from a row gets what
the row knows — `spec` (the spec folder), `task`,
