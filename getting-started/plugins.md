...
**It ships a tab.** *ADO releases* in the run panel of any workflow that `uses: ado-release@1` asks ADO what
...
| `permissions` | `string` | `allow` | What happens when the agent asks permission to use a tool. `allow`: every request inside the allowed folders is allowed once, without asking. `manual`: each request is shown as a card with the runtime's own choices (allow once, for this session, for this workspace, always, deny), and the agent waits for your click. `manual` needs `interactive: true`; with nobody watching, a request is allowed once. Only Copilot reads it for now. A request for a path outside the allowed folders is refused either way. |
...
| `plan` | `string` | | A checklist file whose boxes the session may tick, relative to the working directory. Set, the session gets a `task_done` tool: the agent names a task's id and the engine flips that one box in the file, so the plan never goes through the model to change one character. The spec panel sets it on every implementation chat. Leave unset for a session with no boxes to tick. |
| `checkPlan` | `string` | | A plan file this session is writing, relative to the working directory. Set, the session gets a `check_plan` tool that reads the draft the way the implement phase will and reports every problem by line, so the agent fixes them before it stops. The spec panel sets it on the phase that writes the plan. |
...
turns — a revise from the spec panel's chat box does exactly this — rather than
...
Not by default. To review work before the next task runs, put a
`trigger: manual` task in the file. Every other task uses the same
mechanism. To approve each tool call instead, see
[approving each request yourself](#approving-each-request-yourself).

To set tool policy once for a whole task, use
`availableTools`/`excludedTools`.

**By default, a permission request the agent raises is answered for you,
and the answer is the directory limit above.** Inside the session's
folders it is approved once. Outside them it is refused with a reason the
agent is told. No request is left pending, because a pending request in
an unattended run is a task that hangs until its `timeout:` kills it.

#### Approving each request yourself

`permissions: manual` shows each request to you instead. This is the
same idea as the **Manual** setting in VS Code's own Copilot chat.

| `permissions` | What happens to a request inside the allowed folders |
|---|---|
| `allow` (default) | Allowed once. Nobody is asked. |
| `manual` | Shown as a card in the **AI Session** tab. The agent waits for your click. |

A request for a path outside the allowed folders is refused either way,
before you are asked.

The card only offers the choices the runtime can really keep for that
request:

| Choice | What it means |
|---|---|
| **Allow once** | Only this call. The next one asks again. |
| **Allow for this session** | Every request like this one until the session ends. Not offered for a command Copilot marks as unsafe to remember. |
| **Allow for this workspace** | Saved by Copilot against the repository's git root, for this and every later session there. **Unattended runs in that repository use it too.** |
| **Allow always** | Web addresses only, by domain. Copilot refuses it for anything else, so it is not offered there. |
| **Deny** | The agent is told no. Anything you type after it goes to the agent as the reason. |

Typed text on its own also counts as a deny. Typing never allows
anything.

Things to know:

1. **Only Copilot reads it for now.** On Claude or an ACP agent, every
   request is still allowed once, and the task log says `manual` did
   nothing.
2. **It needs somebody watching.** Without `interactive: true`, `manual`
   works like `allow`, and the log says so. The spec panel sets
   `interactive` on every phase. It does not set it on the first turn of
   an implementation wave, so that turn does not ask.
3. **Stopping the turn with a card open** closes the card, and the call
   does not run.
...
| `ui` | `{ label, entry?, surface? }` — the plugin's own page, shown as a tab in the run panel, the spec panel, or both. See [Plugin apps](https://local-workflows.github.io/getting-started/plugin-apps.md) |
