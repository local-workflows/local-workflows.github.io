...
- **No `node_modules`.** Skipped via the workspace's `.gitignore`, or the
  [`defaultFoldersToIgnoreScanning`](https://local-workflows.github.io/getting-started/settings.md#defaultfolderstoignorescanning)
  built-in list when there is no `.gitignore` — and a `.gitignore` that
  exists but never mentions `node_modules` means it does get scanned.
...
- **No tool folders.** `.yarn`, `.pnpm`, `.turbo`, `.next`, `.nuxt`,
  ...
  `.pnpm-store`, `.debug`, `.git`. Yarn Berry is the loud case: a few hundred
