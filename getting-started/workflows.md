...
| `options` | list | The only accepted values, offered as a pick list — pick-many for a `list`. Read as text. Not for a `map`, not beside `items`. |
| `items` | `file` | `folder` | For a `list` only: every item is a path, picked several at once in the open dialog. Not beside `options`. |
| `root` | `string` | Where the open dialog starts, relative to the workspace folder. Only for a param that opens one. Without it the dialog opens beside the last answer, else at the workspace folder. |
| `filters` | list | Which files the open dialog offers, as patterns like `*.md`. Only for a param picking files. |
