...
run panel of every file that `uses:` the plugin (and in the spec panel
when `sdd.uses` names it), is there whether or not its step is
...
| | |
|---|---|
| **Where it shows** | A tab in the panel of whatever runs the plugin, beside Logs / Artifacts / Problems: the run panel of any `tasks.yml` or workflow that `uses:` it in at least one task, and the spec panel when `sdd.uses` in settings.json names it (that is the one plugin a spec runs on; the default `ai@1` has no page, so a spec panel shows no app tab unless the team points `sdd.uses` at its own plugin). A panel for something that never calls the plugin gets no tab. One tab per plugin that declares a `ui`. `ui.surface` narrows it to one kind of panel. |
| **When it shows** | From the moment the panel opens. Before a run, during, after. It does not depend on a step being selected or running. |
...
10. **Test**: Reload Plugins → open a task that `uses:` the plugin → the tab is there → click through every button → run the task → the state and table update. Then the same in a spec panel, with `sdd.uses` pointed at the plugin, if the page claims to work there.
