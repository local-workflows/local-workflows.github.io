...
1. **A question** always waits for an answer.
2. **A permission card** waits only under `permissions: manual`, which
   is the **Manual approvals** choice in the spec panel. Under the
   default, `allow`, each request is answered for you: approved inside
   the session's own folders, refused outside them. Nothing waits on
   one.

Select the task and open its **AI Session** tab. The question or card is
drawn there, and the tab is marked while it waits, so you can find the
right task from the Logs tab. Answer it and the turn carries on.
