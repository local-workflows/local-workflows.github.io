...
- **The process itself is a file, not a product decision.** Three
  phases, two, your own shape entirely — same engine, same gates. The
  prompt that drives each phase lives in your repository: you edit it,
  version it, and review it in pull requests like any other code —
  instead of re-pasting the same mega-prompt into a chat window and
  hoping everyone on the team uses the same one.
...
- name: Publish
  trigger: manual            # a human reads the resolved draft first
  env:
    NOTES: ${{ run.context.NOTES.summary }}   # as env, so no quoting can break it
  shell: pwsh
  run: ./publish.ps1 -Body $env:NOTES
```

`ai` here is a name you declare, not a built-in — it points at
[`ai@1`](https://local-workflows.github.io/getting-started/plugins.md#the-eight-that-ship), the agent plugin, with
